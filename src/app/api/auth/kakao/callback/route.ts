...
export async function POST(request: NextRequest) {
  try {
    ...
    const backendUrl = `${process.env.NEXT_PUBLIC_BASE_URL}/authorization/kakao/web?${params.toString()}`;
    ...
    if (!backendResponse.ok) {
      ...
      return NextResponse.json(
        {
          message: errorData.message || "카카오 인증 처리 중 오류가 발생했습니다.",
          status: backendResponse.status,
          details: errorData,
        },
        { status: backendResponse.status }
    ...
    console.error("카카오 콜백 API 최상위 오류:", error);
