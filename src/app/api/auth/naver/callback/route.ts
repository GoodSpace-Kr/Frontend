...
export async function POST(request: NextRequest) {
  try {
    ...
    const backendUrl = `${process.env.NEXT_PUBLIC_BASE_URL}/authorization/naver/web?${params.toString()}`;
    ...
    if (!backendResponse.ok) {
      ...
      return NextResponse.json(
        {
          message: errorData.message || "네이버 인증 처리 중 오류가 발생했습니다.",
          status: backendResponse.status,
          details: errorData,
        },
        { status: backendResponse.status }
    ...
    console.error("네이버 콜백 API 최상위 오류:", error);
